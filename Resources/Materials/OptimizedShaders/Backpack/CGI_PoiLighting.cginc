...
void calculateBasePassLightMaps()
{
        ...
        switch(float(0))
        {
            ...
            {
                poiLight.rampedLightMap = saturate(1 - smoothstep(float(1) - .000001, float(0.5), 1 - poiLight.lightMap));
        ...
        if (float(5) == 2) // Wrapped
        {
            ...
            {
                poiLight.rampedLightMap = lerp(float4(1,1,1,1) * lerp(poiLight.indirectLighting, 1, float(0)), float3(1, 1, 1), saturate(1 - smoothstep(float(1) - .000001, float(0.5), 1 - poiLight.lightMap)));
...
float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
{
    ...
    else //if(float(1) == 2) // Wrapped
    {
        ...
        poiLight.rampedLightMap = lerp(float4(1,1,1,1), float3(1, 1, 1), saturate(1 - smoothstep(float(1) - .000001, float(0.5), 1 - uv)));
